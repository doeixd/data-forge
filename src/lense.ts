...
// Convenience functions for creating cells with traits
function string(world: World, ...traitNames: string[]): Cell<string> {
  let cell = createString(world, `string_${Math.random().toString(36).substr(2, 9)}`);
...
function number(world: World, ...traitNames: string[]): Cell<number> {
  let cell = createNumber(world, `number_${Math.random().toString(36).substr(2, 9)}`);
...
type RelationshipDefinition<T, R> = {
